  ...
  try {
    ...
    if (error) {
      if (error.message.includes('Email not confirmed') || error.message.includes('email not confirmed')) {
        const { data: userList } = await supabase.from('User').select('id').eq('email', email).limit(1)
        ...
        return NextResponse.json({ error: 'Email non confirmé. Réessayez.' }, { status: 403 })
      }
