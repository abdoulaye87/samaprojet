...
export async function POST(req: NextRequest) {
  try {
    ...
    await supabase.from('User').upsert(
      { id: authData.user.id, name, email, cash: 0, type: 'player' },
      { onConflict: 'id' }
    )
    ...
    if (loginData?.session && loginData?.user) {
      return NextResponse.json({
        success: true,
        session: { access_token: loginData.session.access_token, refresh_token: loginData.session.refresh_token, expires_in: loginData.session.expires_in },
        user: { id: loginData.user.id, email: loginData.user.email, name, type: 'player' },
      })
    }
